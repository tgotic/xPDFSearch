...
typedef long volatile GAtomicCounter;

// Increment *counter by one and return the final value (after the
// increment).
static inline long gAtomicIncrement(GAtomicCounter *counter) {
  long newVal;
...
// decrement).
static inline long gAtomicDecrement(GAtomicCounter *counter) {
  long newVal;
