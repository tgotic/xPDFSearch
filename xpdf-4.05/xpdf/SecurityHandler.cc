...
StandardSecurityHandler::StandardSecurityHandler(PDFDoc *docA,
						 Object *encryptDictA):
  SecurityHandler(docA), permFlags(0), ownerPasswordOk(gFalse), 
  fileKey(), fileKeyLength(0), encVersion(-1), encRevision(-1),
  encAlgorithm(cryptRC4), encryptMetadata(gFalse), ownerKey(NULL),
  userKey(NULL), ownerEnc(NULL), userEnc(NULL), fileID(NULL), ok(gFalse)
{
