...
OptionalContentGroup::OptionalContentGroup(Ref *refA, TextString *nameA,
  ...
  : ref(*refA), name(nameA), viewState(viewStateA), printState(printStateA)
  , state(gTrue), inViewUsageAppDict(gFalse)
{
}
...
OCDisplayNode::OCDisplayNode(GString *nameA) 
  : name(new TextString(nameA)), ocg(NULL), parent(NULL), children(NULL)
{
}

OCDisplayNode::OCDisplayNode(OptionalContentGroup *ocgA) 
  : name(new TextString(ocgA->name)), ocg(ocgA), parent(NULL), children(NULL)
{
}
