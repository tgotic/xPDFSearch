...
PageTreeNode::PageTreeNode(Ref refA, int countA, PageTreeNode *parentA) 
  : ref(refA), count(countA), parent(parentA), kids(NULL), attrs(NULL)
{
}
...
public:
  ...
  PageLabelNode(const PageLabelNode&) = delete;
  PageLabelNode& operator=(const PageLabelNode&) = delete;
  ~PageLabelNode();
...
GBool Catalog::usesJavaScript() {
  ...
  // look for JavaScript actions in Page.AA
  if (!usesJS) {
    char *touchedObjs = (char *)gmalloc(xref->getNumObjects());
    memset(touchedObjs, 0, xref->getNumObjects());
    Object pagesObj;
