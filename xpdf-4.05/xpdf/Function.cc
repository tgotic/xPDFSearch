...
Function::Function() 
: m(0), n(0), domain(), range(), hasRange(gFalse)
{
}
...
GBool Function::init(Dict *dict) {
  ...
  if (m <= 0) {
	  error(errSyntaxError, -1,
	  "Functions with less than {0:d} inputs are unsupported",
	  1);
	  goto err2;
  }  
  for (i = 0; i < m; ++i) {
...
SampledFunction::SampledFunction(Object *funcObj, Dict *dict) {
  ...
  int bits;
  Guint s;
  double in[funcMaxInputs] = { 0.0 };
  unsigned int j, t, bit, idx;
  ...
  for (int i = 0; i < (1<<m); ++i) {
    idx = 0;
    for (j = m - 1, t = i; (j >= 1) && (j < funcMaxInputs); --j, t <<= 1) {
      if (sampleSize[j] == 1) {
	bit = 0;
      } else {
	bit = (t >> (m - 1)) & 1;
...
PostScriptFunction::PostScriptFunction(PostScriptFunction *func) 
  : codeString(NULL), code(NULL), codeLen(0), codeSize(0), cacheIn(), cacheOut(), ok(gFalse)
{
  if (func && (func != this)) {
