...
public:
  ...
  void mungeName3(GString *aname, GBool *bold, GBool *italic);
...
void SysFontInfo::mungeName3(GString *aname, GBool *bold, GBool *italic) {
  *bold = gFalse;
  *italic = gFalse;
  int n = aname->getLength();
  while (1) {
    if (n >= 4 && !strcmp(aname->getCString() + n - 4, "BOLD")) {
      aname->del(n - 4, 4);
      n -= 4;
      *bold = gTrue;
    } else if (n >= 6 && !strcmp(aname->getCString() + n - 6, "ITALIC")) {
      aname->del(n - 6, 6);
      n -= 6;
      *italic = gTrue;
    } else if (n >= 7 && !strcmp(aname->getCString() + n - 7, "REGULAR")) {
      aname->del(n - 7, 7);
      n -= 7;
    } else if (n >= 5 && !strcmp(aname->getCString() + n - 5, "ROMAN")) {
      aname->del(n - 5, 5);
      n -= 5;
    } else {
      break;
...
static void getWinFontDir(char *winFontDir) {
  ...
  if ((shell32Lib = LoadLibraryA("shell32.dll"))) {
      ...
      (FNSHGETSPECIALFOLDERPATH)((LPVOID)
