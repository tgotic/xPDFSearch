...
GfxICCBasedColorSpace::GfxICCBasedColorSpace(int nCompsA, GfxColorSpace *altA,
  ...
  : nComps(nCompsA), alt(altA), iccProfileStream(*iccProfileStreamA)
{
...
GfxColor *GfxIndexedColorSpace::mapColorToBase(GfxColor *color,
					       GfxColor *baseColor) {
  Guchar *p;
  double low[gfxColorMaxComps] = { 0.0 }, range[gfxColorMaxComps] = { 1 };
  int n, i, k;
...
GfxShading::GfxShading(int typeA) 
  : type(typeA), colorSpace(NULL), background(), hasBackground(gFalse)
  , xMin(0), yMin(0), xMax(0), yMax(0), hasBBox(gFalse)
{
}

GfxShading::GfxShading(GfxShading *shading) 
  : type(shading->type), colorSpace(shading->colorSpace->copy())
  , background(), hasBackground(shading->hasBackground)
  , xMin(shading->xMin), yMin(shading->yMin), xMax(shading->xMax)
  , yMax(shading->yMax), hasBBox(shading->hasBBox)
{
  int i;
...
void GfxGouraudTriangleShading::getBBox(double *axMin, double *ayMin,
...
void GfxPatchMeshShading::getBBox(double *axMin, double *ayMin,
				  double *axMax, double *ayMax) {
  double xxMin = 0;
  double yyMin = 0;
  double xxMax = 0;
