...
GfxFontLoc::GfxFontLoc() 
  : locType(gfxFontLocEmbedded), fontType(fontUnknownType), embFontID()
  , path(NULL), fontNum(0), oblique(0.0), encoding(NULL), wMode(0)
  , substIdx(-1)
{
}
...
GfxFont::GfxFont(const char *tagA, Ref idA, GString *nameA,
  ...
  : tag(new GString(tagA)), id(idA), name(nameA), type(typeA)
  , flags(0), embFontName(NULL), embFontID(embFontIDA)
  , fontMat(), fontBBox(), missingWidth(0.0), ascent(0.0)
  , descent(0.0), declaredAscent(0.0), hasToUnicode(gFalse)
  , ok(gFalse)
{
}
...
int Gfx8BitFont::getNextChar(char *s, int len, CharCode *code,
			     Unicode *u, int uSize, unsigned int *uLen,
