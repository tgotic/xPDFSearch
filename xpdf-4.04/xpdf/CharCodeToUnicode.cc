...
CharCodeToUnicode *CharCodeToUnicode::parseUnicodeToUnicode(
  ...
  while (getLine(buf, sizeof(buf), f)) {
	...
	!parseHex(tok, (int)strlen(tok), &u0)) {
      error(errSyntaxWarning, -1,
	    "Bad line ({0:u}) in unicodeToUnicode file '{1:t}'",
	    line, fileName);
      continue;
    }
    n = 0;
...
unsigned int CharCodeToUnicode::parseUTF16String(char *uStr, int n, Unicode *uOut) {
  int i = 0;
  unsigned int uLen = 0;
  while (i < n) {
    Unicode u;
...
unsigned int CharCodeToUnicode::mapToUnicode(CharCode c, Unicode *u, int size) {
  unsigned int i, j;
