...
public:
  ...
  TextCharLine(const TextCharLine&) = delete;
  TextCharLine& operator=(const TextCharLine&) = delete;
...
void TextPage::writeReadingOrder(void *outputStream,
  ...
  GBool primaryLR;
  GString *s;
  int colIdx, parIdx, lineIdx, rot, n, abort = 0;
  ...
  for (colIdx = 0; (colIdx < columns->getLength()) && !abort; ++colIdx) {
    col = (TextColumn *)columns->get(colIdx);
    for (parIdx = 0; (parIdx < col->paragraphs->getLength()) && !abort; ++parIdx) {
      par = (TextParagraph *)col->paragraphs->get(parIdx);
      for (lineIdx = 0; (lineIdx < par->lines->getLength()) && !abort; ++lineIdx) {
	...
	abort = (*outputFunc)(outputStream, s->getCString(), s->getLength());
	delete s;
      }
      if (!abort)
        abort = (*outputFunc)(outputStream, eol, eolLen);
    }
    if (!abort)
      abort = (*outputFunc)(outputStream, eol, eolLen);
  }
...
void TextPage::writePhysLayout(void *outputStream,
  ...
  GList *columns;
  GBool primaryLR;
  int ph, colIdx, parIdx, lineIdx, rot, y, i, abort = 0;
  ...
  for (i = 0; (i < ph) && !abort; ++i) {
    if (out[i]) {
      abort = (*outputFunc)(outputStream, out[i]->getCString(), out[i]->getLength());
      delete out[i];
    }
    if (!abort)
      abort = (*outputFunc)(outputStream, eol, eolLen);
  ...
  if (overlappingChars && !abort) {
    if (overlappingChars->getLength() > 0) {
      TextColumn *col1 = buildOverlappingTextColumn(overlappingChars);
      abort = (*outputFunc)(outputStream, eol, eolLen);
      for (parIdx = 0; (parIdx < col1->paragraphs->getLength()) && !abort; ++parIdx) {
        par = (TextParagraph *)col1->paragraphs->get(parIdx);
        for (lineIdx = 0; (lineIdx < par->lines->getLength()) && !abort; ++lineIdx) {
	  line = (TextLine *)par->lines->get(lineIdx);
	  ...
	  delete s;
        ...
        if ((parIdx < col1->paragraphs->getLength() - 1) && !abort) {
          abort = (*outputFunc)(outputStream, eol, eolLen);
	}
      }
      delete col1;
...
void TextPage::writeSimpleLayout(void *outputStream,
  ...
  for (i = 0; (i < superLines->getLength()) && !abort; ++i) {
    ...
    abort = (*outputFunc)(outputStream, out->getCString(), out->getLength());
    delete out;
    if (!abort) {
      abort = (*outputFunc)(outputStream, eol, eolLen);
      if ((i + 1 < superLines->getLength()) && !abort) {
...
void TextPage::writeSimple2Layout(void *outputStream,
  ...
  GString *out;
  GBool primaryLR;
  int colIdx, parIdx, lineIdx, abort = 0;
...
void TextPage::writeLinePrinter(void *outputStream,
  ...
  i = 0;
  j = chars->getLength() - 1;
  for (y = yShift; (y < pageHeight) && !abort; y += lineSpacing) {
...
void TextPage::writeRaw(void *outputStream,
  ...
  for (i = 0; (i < chars->getLength()) && !abort; ++i) {
...
TextLine *TextPage::buildLine(GList *charsA, int rot,
  ...
  int dir, dir2;
  GBool rotated1, spaceAfter, spaceBefore;
  int i, j;
  ...
  while (i < charsA->getLength()) {
    ...
    dir = getCharDirection(
	      ...
	      (i < charsA->getLength() - 1) ? (TextChar *)charsA->get(i+1)
	                                    : (TextChar *)NULL);
    rotated1 = ((TextChar *)charsA->get(i))->rotated;
	...
	break;
	  ...
	  (control.splitRotatedWords && ch2->rotated != rotated1) ||
	  (dir && dir2 && dir2 != dir) ||
    ...
    word = new TextWord(charsA, i, j - i, rot, rotated1, dir,
...
static int outputToFile(void *stream, const char *text, int len) {
...
TextOutputDev::TextOutputDev(char *fileName, TextOutputControl *controlA,
  ...
  :  outputFunc(NULL), outputStream(NULL), control(*controlA)
  , text(new TextPage(&control)), needClose(gFalse), ok(gTrue)
{
  // open file
  if (fileName) {
    if (!strcmp(fileName, "-")) {
...
TextOutputDev::TextOutputDev(TextOutputFunc func, void *stream,
  ...
  : outputFunc(func), outputStream(stream), control(*controlA)
  , text(new TextPage(&control)), needClose(gFalse), ok(gTrue)
{
  generateBOM();
}
