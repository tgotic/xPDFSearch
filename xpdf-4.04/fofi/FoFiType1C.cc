...
FoFiType1C::FoFiType1C(char *fileA, int lenA, GBool freeFileDataA):
  ...
  name(NULL), encoding(NULL), nameIdx(), topDictIdx(), stringIdx(),
  gsubrIdx(), charStringsIdx(), topDict(), privateDicts(NULL), 
  nGlyphs(0), nFDs(0), fdSelect(NULL), charset(NULL), gsubrBias(0),
  parsedOk(gFalse), ops(), nOps(0), nHints(0), firstOp(gFalse), openPath(gFalse)
{
}
...
GHash *FoFiType1C::getNameToGIDMap() {
  ...
  for (gid = 0; gid < nGlyphs; ++gid) {
    ok = gTrue;
    getString(charset[gid], xname, &ok);
    if (ok) {
      map->add(new GString(xname), gid);
    }
  }
  return map;
}
