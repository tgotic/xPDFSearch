...
IsWindowsPlatformIdOrGreater(DWORD dwPlatformId)
{
    ...
    return (VerifyVersionInfoW(&osvi, VER_PLATFORMID, dwlConditionMask) != FALSE);
}
#endif
...
GBool pathIsFile(const char *path) {
  ...
  fileNameToUCS2(path, wPath, _MAX_PATH + 1);
...
FILE *openFile(const char *path, const char *mode) {
  ...
  fileNameToUCS2(path, wPath, _MAX_PATH + 1);
  for (i = 0; (i < sizeof(wMode) / sizeof(wchar_t) - 1) && mode[i]; ++i) {
...
#ifdef _WIN32
  wchar_t wPath[_MAX_PATH + 1];
  return _wmkdir(fileNameToUCS2(path, wPath, _MAX_PATH + 1));
#else
