...
public:
  ...
  GFileOffset get(int idx) const { return tab[idx]; }
...
private:
  ...
  int size{ 16 };
  int len{ 0 };
  GFileOffset* tab{(GFileOffset *)gmallocn(size, sizeof(GFileOffset))};
};
...
GBool XRef::getEncryption(int *permFlagsA, GBool *ownerPasswordOkA,
			  ...
			  CryptAlgorithm *encAlgorithmA) const {
  if (!encrypted) {
    return gFalse;
  }
