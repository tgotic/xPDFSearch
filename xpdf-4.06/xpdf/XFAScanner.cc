...
void XFAScanner::scanNode(ZxElement *elem,
  ...
  if (nodeName) {
    ...
    int idx = childNameIdx->lookupInt(nodeName);
    childNameIdx->replace(nodeName, idx + 1);
  ...
  if (nodeFullName) {
    ...
    int idx = childFullNameIdx->lookupInt(nodeFullName);
    childFullNameIdx->replace(nodeFullName, idx + 1);
    childFullName->appendf("[{0:d}]", idx);
  } else {
